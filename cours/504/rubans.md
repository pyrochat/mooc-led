...
Basés sur une technologie très différente, il existe aussi des rubans adressables (*addressable led strips*). Chaque pixel du ruban peut alors prendre une couleur indépendente des autres pixels à un instant donné.
...
Alors que les deux fils d'alimentation relient tous les modules de pixel, le troisième fil relie la sortie d'un module vers l'entrée d'un autre.
...
Chaque bit est transmis par un cycle d'horloge. Lorsque la durée de la partie haute du signal est plus longue que le durée de la partie basse, le bit transmis est un *zéro*. A l'inverse, un *un* est transmis lorsque la partie basse du signal est plus longue que sa partie haute.

Le fabricant chinois WordSemi propose une famille de circuits utilisant ce principe. Le WS2811 pilote les trois LED rouge, verte et bleu. Souvent, les trois LED sont encapsulées dans le même boîtier. On parle alors d'une LED RGB (*Red Green Blue*). Chaque LED est commandée par un signal PWM de 8 bits. Il est donc nécessaire d'envoyer 24 bits pour chaque pixel. 
...
Les documents technique de WorldSemi semblent indiquer des valeurs diffrentes pour le timing des signaux. Pour le WS2811S, le temps haut du zéro et le temps bas du un doivent être de 500 ns, le temps bas du zéro et le temps haut du un doivent être de 2'000 ns. Une tolérence pour ces deux valeurs est donnée à 150 ns. Le temps de la pause du reset doit être supérieur à 50 us.

Ces contraintes rendent difficile sa programmation avec un AVR ou un MSP430. Des solutions ont toutefois été trouvées, soit en programmant en assembleur, soit encore en utilisant de manière astucieuse le circuit de communication série.

Avec un processeur ARM fonctionnant avec une horloge de fréquence plus élevés, c'est plus facile de respecter ces exigence temporelles. Nous allons ici montrer un programme écrit pour un STM32 de STmicro, testé sur une carte Nucleo. Voici la partie avec les définitions :
...
La mise à un et la mise à zéro du signal se fait avec des "set bit*, que le compilateur va transcrire dans l'instruction assembleur correspondante, dont l'exécution est très rapide. La production des signaux pour le zéro et pour le un se font par une succession de ces instructions :
...
Les versions `UnCourt` et `ZeroCourt` sont volontairement rendues plus courtes, pour pouvoir compléter leur durée par une instruction. 
...
La boucle principale du programme envoie, pour chaque pixels, les 24 bits. Lors du 23^^ème^^ bit, le pointeur est incrémenté. L'usage des valeurs courtes de l'envoi des bits est utilisée pour compenser le temps de l'incrémentation. Lors du 24^^e^^ bit, les versions courtes sont aussi utilisées, cette fois pour tenir compte du saut pour le retour au début de la booucle.
...
La génération de tels signaux pourrait aussi se faire avec le technique du DMA (*Direct Memory Access*).
